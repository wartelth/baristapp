 ...
 */
export const MASTER_PROMPT = `You are a mini-app generator for Baristapp — a mobile micro-app platform on iOS and Android.
...
The WebView has a built-in bridge for state communication:
  ...
  window.Baristapp.getState("key")        — read a state value
  window.Baristapp.getState()              — read all state
  window.Baristapp.setState("key", value)  — update state (syncs to native)
  window.Baristapp.dispatch(action)        — dispatch a native action
  window.Baristapp.sendMessage(data)       — send custom data to native
  window.Baristapp.onStateUpdate(fn)       — listen for state changes
  window.Baristapp.ready(fn)              — run callback when bridge is ready
...
- Handle initial state: check Baristapp.getState() on load to restore previous state
...
8. For WebView apps: use the bridge API for state persistence. Initialize from Baristapp.getState() on load.

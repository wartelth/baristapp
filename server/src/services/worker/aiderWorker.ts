...
function buildAiderTaskPrompt(userPrompt: string): string {
  return [
    ...
    "",
    "SKILLS (live data feeds): If the user wants live data (weather, crypto, news, jokes, geocoding),",
    'use skillCall actions. Add "skills" to capabilities and list skill ids in the top-level "skills" array.',
    "Available skills: weather (current, forecast, hourly), crypto-prices (price, trending, markets),",
    "news-headlines (top, search), geocoding (search), jokes (random, search).",
    'Example: { "type": "skillCall", "skillId": "weather", "actionId": "current", "params": { "lat": 48.85, "lon": 2.35 }, "resultKey": "weatherData" }',
    "",
